...
defmodule Speechly.Slu.V1.SLUConfig.Option do
  @moduledoc false
  defstruct key: "", value: [], __uf__: []
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(Speechly.Slu.V1.SLUConfig.Option))
        end
      )
    )
        ...
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {2, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[value: msg.value ++ [delimited]], rest}
  ...
  (
    ...
    def defs() do
      %{1 => {:key, {:scalar, ""}, :string}, 2 => {:value, :unpacked, :string}}
    end
    ...
    def defs_by_name() do
      %{key: {1, {:scalar, ""}, :string}, value: {2, :unpacked, :string}}
    end
  )
